...
module NomadClient
  class SchedulerConfiguration
    ...
    attr_accessor :memory_oversubscription_enabled
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NomadClient::SchedulerConfiguration` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NomadClient::SchedulerConfiguration`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'memory_oversubscription_enabled')
        self.memory_oversubscription_enabled = attributes[:'memory_oversubscription_enabled']
      end
      ...
      if attributes.key?(:'scheduler_algorithm')
        self.scheduler_algorithm = attributes[:'scheduler_algorithm']
      end
    end
